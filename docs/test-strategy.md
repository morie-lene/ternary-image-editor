...
右`mouseClick`を送る動的事故回帰を同じ公開包装面へ置く。[test_packaging.py](../tests/test_packaging.py)の一部はWindows構築scriptの必須入力、
...
名前と実体も分ける。`0.8.0`は応用版、JSONの`schema_version`は出力形、wheel SHA-256は一構築物、
